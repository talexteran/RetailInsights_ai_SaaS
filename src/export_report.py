...
Retorna el HTML como string (sin escribir a disco).
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .verde  .dot { background:var(--success); }
  .amarillo .dot { background:var(--warning); }
  .rojo   .dot { background:var(--danger); }
  ...
  .rec-top { background:rgba(59,130,246,0.1); border:1px solid rgba(59,130,246,0.3); border-radius:12px; padding:1.5rem; margin-bottom:2rem; }
...
<div class="wrapper">
  ...
  <div class="chart-grid">
    {% for html in chart_html %}
    <div class="chart-card">{{ html }}</div>
    {% endfor %}
  </div>
  {% endif %}
  ...
  <footer>
    ...
    <p>Powered by Python · Plotly · Claude AI</p>
  </footer>
...
def export_report(
    ...
    output_path: str = None,   # ignorado — mantenido por compatibilidad CLI
) -> str:
    """
    Genera el reporte HTML y lo retorna como string.
    No escribe nada a disco (compatible con Streamlit Cloud).
    """
    # Convertir figuras Plotly a HTML embebido (sin JS externo, usa CDN del template)
    chart_html = []
    for name, fig in charts.items():
        try:
            html_div = pio.to_html(
                fig,
                full_html=False,
                include_plotlyjs=False,  # ya está en el template via CDN
                config={"displayModeBar": False},
            )
            chart_html.append(html_div)
        except Exception:
            chart_html.append(f"<p style='color:#64748b'>Gráfico no disponible: {name}</p>")
